...
from sub_system import shutdown #Import shutdown
...
def enforce_robot_laws(environmental_data, social_health_data=None, robot_actions=None):
    ...
    if environmental_data:
        ...
        if environmental_data.get('radiation') and environmental_data['radiation'].get('alert'):
             ...
             if environmental_data.get('radiation').get('level') > 100: # Example radiation level check
                 shutdown("Critical radiation level detected, possible harm to humans.") #shutdown if high radiation.
    ...
    if social_health_data:
        ...
        if social_health_data.get('crime_rate', 0) > 10:
            ...
            if social_health_data.get('violence_level', 0) > 5: #Example violence level check
                shutdown("High violence level detected, possible harm to humans.") #shutdown if high violence.
